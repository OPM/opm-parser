...
BOOST_AUTO_TEST_CASE(PORV_cartesianDeck) {
    ...
    auto poro = state->getEclipseProperties().getDoubleGridProperty("PORO");
    BOOST_CHECK( poro->containsNaN() );
    BOOST_CHECK_THROW( state->getEclipseProperties().getDoubleGridProperty("PORV") , std::logic_error );
}
...
BOOST_AUTO_TEST_CASE(PORV_initFromPoro) {
    ...
    auto porv = state->getEclipseProperties().getDoubleGridProperty("PORV");
...
BOOST_AUTO_TEST_CASE(NAKED_GRID_THROWS) {
    ...
    auto state = std::make_shared<Opm::EclipseState>(deck , Opm::ParseContext());
    BOOST_CHECK_THROW( state->getEclipseProperties().getDoubleGridProperty("PORV") , std::invalid_argument );
}
