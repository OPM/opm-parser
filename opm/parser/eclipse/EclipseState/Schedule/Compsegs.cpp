    ...
    std::vector<std::shared_ptr<Compsegs>> Compsegs::compsegsFromCOMPSEGSKeyword(DeckKeywordConstPtr compsegsKeyword,
        ...
        for (int recordIndex = 1; recordIndex < int(compsegsKeyword->size()); ++recordIndex) {
            ...
            if (record->getItem("DISTANCE_START")->hasValue(0)) {
                distance_start = record->getItem("DISTANCE_START")->getRawDouble(0);
            ...
            if (record->getItem("DISTANCE_END")->hasValue(0)) {
                distance_end = record->getItem("DISTANCE_END")->getRawDouble(0);
            } else {
            ...
            double center_depth;
            if (record->getItem("CENTER_DEPTH")->hasValue(0)) {
                center_depth = record->getItem("CENTER_DEPTH")->getRawDouble(0);
            } else {
                center_depth = 0.;
            }
            ...
            if (record->getItem("THERMAL_LENGTH")->hasValue(0)) {
