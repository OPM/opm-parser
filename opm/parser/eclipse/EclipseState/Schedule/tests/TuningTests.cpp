...
BOOST_AUTO_TEST_CASE(TuningTest) {
  ...
  /********** Test updating of defaults ***************/
  tuning->updateDefaults("TSINIT", 7 * Metric::Time);
  timestep = 1;
  double new_default = tuning->getTSINIT(timestep);
  BOOST_CHECK_CLOSE(new_default, 7 * Metric::Time, diff);
  timestep = 4;
  new_default = tuning->getTSINIT(timestep);
  BOOST_CHECK_CLOSE(new_default, 7 * Metric::Time, diff);
  timestep = 5;
  TSINIT = tuning->getTSINIT(timestep);
  BOOST_CHECK_CLOSE(TSINIT, 2 * Metric::Time, diff);
