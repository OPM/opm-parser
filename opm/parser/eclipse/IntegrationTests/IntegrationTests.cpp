...
BOOST_AUTO_TEST_CASE(parse_fileWithWWCTKeyword_deckReturned) {
    ...
    BOOST_CHECK_NO_THROW( ecl::parseDeck( parser, singleKeywordFile.string(), ParseContext()) );
}
...
BOOST_AUTO_TEST_CASE(parse_stringWithWWCTKeyword_deckReturned) {
    ...
    BOOST_CHECK( parser.isRecognizedKeyword("WWCT"));
    BOOST_CHECK( parser.isRecognizedKeyword("SUMMARY"));
    BOOST_CHECK_NO_THROW(ecl::parseDeckString( parser, wwctString, ParseContext()));
}
...
BOOST_AUTO_TEST_CASE(parse_fileWithWWCTKeyword_deckHasWWCT) {
    boost::filesystem::path singleKeywordFile("testdata/integration_tests/wwct.data");
    auto parser = createWWCTParser();
    auto deck = ecl::parseDeck( parser, singleKeywordFile.string(), ParseContext());
...
BOOST_AUTO_TEST_CASE(parse_unknownkeyword_exceptionthrown) {
    Parser parser;
    BOOST_CHECK_THROW( ecl::parseDeck( parser, "testdata/integration_tests/someobscureelements.data", ParseContext()), std::invalid_argument);
}
...
BOOST_AUTO_TEST_CASE(parse_truncatedrecords_deckFilledWithDefaults) {
    Parser parser;
    auto deck =  ecl::parseDeck( parser, "testdata/integration_tests/truncated_records.data", ParseContext());
